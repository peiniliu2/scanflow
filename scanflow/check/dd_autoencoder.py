...
def get_checker(X_train, X_test, paths, date=None):
    np.random.seed(42)
    tf.random.set_seed(42)
    ...
    X_train = scale(paths['ad_checker_scaler_dir'], X_train)
    X_test = scale(paths['ad_checker_scaler_dir'], X_test)
    X_full = pd.concat([X_train, X_test], sort=False)

    X_train_noisy = add_noise(X_train)

    model_path = os.path.join(paths['ad_checker_model_dir'], 'saved_model.pb')
    if os.path.isfile(model_path):
        ddae_model = tf.keras.models.load_model(paths['ad_checker_model_dir'])
    else:
        ...
        ddae_history = ddae_model.fit(np.array(X_train_noisy), np.array(X_train),
                                      ...
                                      verbose=0)
        ddae_model.save(paths['ad_checker_model_dir'], save_format='tf')
